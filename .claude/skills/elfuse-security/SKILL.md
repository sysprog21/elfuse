...
- `scripts/check-atomics.py` fails a C11 atomic operation written without the
  ...
  the site. It does not see plain-operator access to an `_Atomic` object,
  which is a review question rather than a gap to skip and the memory-order
  case worth the budget; `elfuse-conventions` says why a regex cannot close it.
