...
func TestUnpackFreshFixtures(t *testing.T) {
	for _, c := range []struct {
		...
		file    string
		want    string
		destDir func(t *testing.T) string
	}{
		...
		{name: "trailing separator", layers: [][]tarEntry{{{Name: "f", Body: "x"}}},
			file: "f", want: "x",
			destDir: func(t *testing.T) string {
				return filepath.Join(t.TempDir(), "out") + string(filepath.Separator)
			}},
	} {
...
// Losing the publication race is benign only for the content-addressed cache.
// An explicit rootfs must not report success when its tree was discarded.
func TestUnpackFreshLostRenameRace(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			// A non-empty directory makes the rename fail the way a peer that
...
// The cache lives inside the store, so it follows the store's private mode and
// its abandoned staging trees are swept.
func TestUnpackCachePrivateModeAndTempSweep(t *testing.T) {
	...
	base := filepath.Join(s.root, cacheRootfs, "sha256")
...
// A symlink to the rootfs directory is followed.
...
// A destination whose parent chain already holds a symlink on disk must be
// resolved against the disk, not only against symlinks seen in this layer.
...
// A lookup must not create the store.
...
// A multi-megabyte layer must be interruptible while it is being applied, not
// only at the boundary between layers, and the cancellation must be reported
// as such when go-archive decompresses through an unpigz child.
func TestUnpackIntoCancelsMidLayer(t *testing.T) {
	var entries []tarEntry
	for i := range 400 {
		entries = append(entries, tarEntry{Name: fmt.Sprintf("f%04d", i), Body: incompressibleBody(i, 32*1024)})
	}
	s, d := storeWithImage(t, "cancelmid:1", testImage{layers: [][]tarEntry{entries}})
	...
	for _, c := range []struct {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			if c.name == "read-only directory" {
				...
				// Permit the testing package to remove the fixture.
				os.Chmod(filepath.Join(dest, "ro"), 0o755)
			}
		})
	}
}
