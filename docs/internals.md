...
the arena cursor rewinds in place instead of taking a refill HVC. EL1 clears
the descriptors itself and advances a page-table epoch in shim_data; the host
translation cache checks that epoch alongside its own generation counter, so a
host thread that never takes `mmap_lock` stops translating the retired range at
once.
