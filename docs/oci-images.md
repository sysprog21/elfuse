...
at a cache path is rejected, and a staging tree abandoned by an interrupted
unpack is removed once it is a day old. A cached tree is reused as is; remove
it to unpack again.
...
and losing that rename is an error. A destination inside the store is
rejected.

Layers are applied in manifest order by `moby/go-archive`, which handles
whiteouts, hardlinks, path containment, file metadata, gzip, and zstd.
...
- Absolute symlink targets are rewritten relative to the link's on-disk
  parent; a hardlink to such a symlink is rebased at its own location.
- Directory modes are restored after all layers are applied, so a read-only
  directory still receives later entries.
- Setuid, setgid, and sticky bits are cleared, since ownership is never
  applied.
