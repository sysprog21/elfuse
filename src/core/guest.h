...
typedef struct {
    ...
    /* Page table generation counter: incremented on every host PT modification.
     * The per-thread GVA TLB cache checks it together with the EL1 PT epoch at
     * SHIM_MMAP_PT_EPOCH_OFF, which EL1 fast munmap bumps in its place. 64-bit
     * to avoid wrap-around stale hits over long-running sessions.
     */
...
/* Bump the page table generation counter (call after any host PT modification).
...
/* Materialize lazy blocks intersecting [gva, gva+len). Takes mmap_lock; call
 * only without locks that nest below it. Later access under those locks must
 * use nofault helpers: pre-faulting neither pins mappings nor guarantees
 * success.
 *
...
/* Materialization with mmap_lock already held. Follow with nofault access. */
...
/* Guest-to-host copy without lazy materialization; a missing PTE fails. */
