...
/* EL1 munmap advances this once its descriptor stores and TLBI have completed
 * and before it releases the retire tail. guest_t.pt_gen counts host writers
 * only, so gva_translate_perm keys its cache on both words: without this one a
 * host thread that is not a vCPU thread, and so never drains at a VM exit,
 * keeps serving a retired range until some thread takes mmap_lock. One shared
 * line per PTE-clearing retirement, beside the broadcast TLBI it already pays.
 */
#define SHIM_MMAP_PT_EPOCH_OFF 0x2190u
