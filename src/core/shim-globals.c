...
_Static_assert(SHIM_MMAP_PT_EPOCH_OFF >=
                       SHIM_MMAP_PT_GATE_OFF + sizeof(uint32_t) &&
                   SHIM_MMAP_PT_EPOCH_OFF % sizeof(uint64_t) == 0 &&
                   SHIM_MMAP_PT_EPOCH_OFF + sizeof(uint64_t) <=
                       SHIM_MMAP_CONTROL_BASE,
               "EL1 PT epoch overlaps the PT gate or mmap controls");
...
_Static_assert(SHIM_COUNTER_FUTEX_FAULT_BAIL * 8 == 104, "CB_FUTEX_FAULT_BAIL");
...
static const char *const counter_names[SHIM_COUNTERS_N] = {
    ...
    [SHIM_COUNTER_FUTEX_FAULT_BAIL] = "FUTEX_FAULT_BAIL",
