...
static el1_mmap_context_t el1_context(uint64_t *saved_gprs)
{
    ...
    return (el1_mmap_context_t) {
        ...
        .pt_epoch = (_Atomic uint64_t *) (shim_data + SHIM_MMAP_PT_EPOCH_OFF),
        .slot = slot,
    };
}
...
static bool el1_mmap(el1_mmap_context_t *context, uint64_t *saved_gprs)
{
    ...
    uint32_t credits =
        atomic_load_explicit(&control->metadata_credits, memory_order_relaxed);
    if (credits == 0) {
        el1_gate_leave(context);
        el1_mmap_counter(context, SHIM_MMAP_COUNTER_CAPACITY_MISS);
        return false;
    ...
    entry->len = length;
    entry->prot = prot;
    atomic_store_explicit(&control->metadata_credits, credits - 1,
                          memory_order_relaxed);

    /* Publish the mmap entry before exposing its range to the acquire load in
     * el1_munmap_match_arena. Retirement then carries this publication to the
     * host's acquire snapshot of the retire tail.
     */
    atomic_store_explicit(&control->tail, tail + 1, memory_order_release);
    atomic_store_explicit(&control->cursor, cursor, memory_order_release);
...
static bool el1_munmap(el1_mmap_context_t *context, uint64_t *saved_gprs)
{
    ...
    if (walked && charged_pages != 0) {
        ...
        /* Release pairs with the acquire in gva_translate_perm: a host thread
         * that reads the new epoch walks the cleared descriptors instead of
         * serving its cached translation. pt_gen never sees this path.
         */
        atomic_fetch_add_explicit(context->pt_epoch, 1, memory_order_release);
    }
