 ...
 * is self-contained, needs no external harness, and is meant to be run under
 ...
 * tail remain visible. The isolated driver gives every timed section-A and
 * section-C operation a new elfuse process.
 *
 ...
 * high-water, which is the case lazy_fresh_range skips the stale-PTE scan for.
 ...
 * mapping. Periodic VM exits keep large store loops below elfuse's watchdog.
 */
static bool bench_munmap_dirty_once(uint64_t size,
                                    double timer_ticks,
                                    double *unmap_ns)
{
    volatile uint8_t *p = mmap(NULL, size, PROT_READ | PROT_WRITE,
    ...
    for (uint64_t off = 0; off < size; off += 4 * KIB) {
         ...
         * side-effect-free HVC outside the timed region. It also drains
         * retirement in bounded chunks.
         */
        if (off != 0 && (off & (DIRTY_VMEXIT_STRIDE - 1)) == 0)
            (void) fcntl(-1, F_GETFD);
    }

    /* Establish a clean host-retirement boundary before the measurement. */
    (void) fcntl(-1, F_GETFD);
...
static int bench_munmap_dirty_measure(uint64_t size,
                                      ...
                                      double *max_ns)
{
    double *samples = malloc((size_t) ops * sizeof(*samples));
    if (!samples)
        return 0;

    double warmup_ns;
    if (!bench_munmap_dirty_once(size, timer_ticks, &warmup_ns)) {
        free(samples);
        return 0;
    }
    for (unsigned op = 0; op < ops; op++) {
        if (!bench_munmap_dirty_once(size, timer_ticks, &samples[op])) {
            free(samples);
            return 0;
        }
    }

    qsort(samples, ops, sizeof(*samples), cmp_d);
    *p50_ns = sorted_quantile(samples, ops, 0.50);
    *p95_ns = sorted_quantile(samples, ops, 0.95);
    *max_ns = samples[ops - 1];
    free(samples);
    return 1;
}
...
int main(int argc, char **argv)
{
    ...
    /* One warmed section-C sample for the host-side process distribution. */
    if (argc == 3 && strcmp(argv[1], "c-one") == 0) {
        char *end = NULL;
        errno = 0;
        uint64_t size = strtoull(argv[2], &end, 0);
        if (errno || !end || *end || size == 0)
        ...
        /* Drain the warmup retirement before constructing the timed mapping. */
        (void) fcntl(-1, F_GETFD);

        double munmap_ns = 0;
        if (!bench_munmap_dirty_once(size, timer_ticks, &munmap_ns)) {
            printf("munmap-dirty size=%llu FAILED\n",
                   (unsigned long long) size);
            return 1;
        }
        printf("munmap-dirty size=%llu ops=1 munmap_ns=%.6f\n",
               (unsigned long long) size, munmap_ns);
        return 0;
    }
    if (argc != 1)
        return 2;
