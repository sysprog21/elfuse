...
def read_file(path: Path, suite: Optional[str] = None,
              seen: Optional[List[Path]] = None) -> List[Action]:
    """With no suite, each action's suite comes from its first matcher."""
    seen = list(seen or [])
    if path in seen:
    ...
    for where, entry in _entries(path):
        if isinstance(entry, dict) and "include" in entry:
            if suite is None:
                raise ExpectationError("%s: include is not legal in %s" % (where, FLAKY))
        ...
        if suite is None:
            ...
            entry_suite = ids.suite_of(first) if isinstance(first, str) else ""
        else:
            entry_suite = suite
        action = _parse_action(entry, where, entry_suite)
...
def load(suite: str, backend: str, root: Path) -> Expectations:
    ...
    flaky = root / FLAKY
    if flaky.exists():
        actions.extend(a for a in read_file(flaky) if ids.suite_of(a.matchers[0]) == suite)
...
def lint(root: Path, seeded_ok: bool = False) -> List[str]:
    ...
    for path in sorted(root.glob("*.jsonc")):
        try:
            suite = None if path.name == FLAKY else path.stem.split("_", 1)[0]
            actions = read_file(path, suite)
            if suite and "_" in path.stem:
    ...
    if not seeded_ok:
        problems += ["%s: still carries a seeded reason; triage it" % s for s in seeded]
    # A base file is read again through every leaf that includes it.
