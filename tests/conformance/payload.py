...
from conformance import EXIT_DRIFT, EXIT_OK, EXIT_USAGE
...
def fingerprint(pin_section: Dict[str, Any], files: Sequence[Path], flavor: str = "") -> str:
    """Hash by basename and in the given order, so inputs need distinct names."""
    h = hashlib.sha256()
...
def absent_message(suite: str, root: Path, state: str, build_hint: str) -> str:
    return ("%s payload %s (%s); run: %s, see docs/conformance.md"
...
def check_pins(doc: Any, schema: Dict[str, Dict[str, str]]) -> Dict[str, Any]:
    ...
    for section, fields in schema.items():
        ...
        for name, kind in fields.items():
            ...
            if kind not in ("int", "url", "str") and kind not in _HEX:
                raise PinError("%s: unknown kind %r in schema" % (where, kind))
            if kind == "int":
...
def diff_pins(old: Dict[str, Any], new: Dict[str, Any]) -> List[str]:
    out = []
...
def refresh(provider: Any, ref: Optional[str] = None, check: bool = False,
            ...
            fail: Callable[[str], None] = print) -> int:
    """Refresh pins through the provider schema and latest_pin hook."""
    try:
        current = load_pins(provider.pins_path, provider.pins_schema)
        fresh = provider.latest_pin(copy.deepcopy(current), ref)
        check_pins(fresh, provider.pins_schema)
    except (UpdateError, OSError, KeyError, IndexError, ValueError) as e:
        # urllib failures are OSError; a malformed upstream response raises
        # Key/Index/ValueError out of latest_pin.
        fail(str(e))
        return EXIT_USAGE
    changes = diff_pins(current, fresh)
    if not changes:
    ...
    for line in changes:
        out("  " + line)
    if check:
        out("%s: upstream has moved; run: scripts/conformance pins update %s"
            % (provider.pins_path, provider.name))
        return EXIT_DRIFT
    try:
        write_pins(provider.pins_path, fresh, provider.pins_schema)
    except OSError as e:
        fail(str(e))
        return EXIT_USAGE
