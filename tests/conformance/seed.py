...
def propose(cases: List[CaseResult], reason: str, bootstrap: bool,
    ...
    for c in cases:
        if c.status is Status.ERROR:
            raise SeedError("%s is a harness ERROR; seeding refuses it" % c.id)
        if bootstrap:
...
def format_actions(actions: List[Dict[str, Any]], header: str = "") -> str:
    ...
    for a in actions:
        if "include" in a:
            ...
            continue
        lines.append('    { "type": %s,' % json.dumps(a["type"]))
        for key in ("reason", "since", "tracking"):
            if a.get(key):
                lines.append('      "%s": %s,' % (key, json.dumps(a[key])))
        matchers = a["matchers"]
        if len(matchers) == 1:
...
def append(leaf: Path, actions: List[Dict[str, Any]]) -> None:
    """Append actions new to the leaf, preserving the leading comments."""
    ...
    present = {(a["type"], m) for a in existing if "type" in a for m in a["matchers"]}
    fresh = []
    for a in actions:
        matchers = [m for m in a["matchers"] if (a["type"], m) not in present]
        if matchers:
            fresh.append(dict(a, matchers=matchers))
    if not fresh:
        return
    payload.atomic_write(leaf, format_actions(existing + fresh, "\n".join(header_lines)))
