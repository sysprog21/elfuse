...
def parse(doc: Any, source: str) -> Selection:
    ...
    declined_doc = doc.get("declined", [])
    if not isinstance(declined_doc, list):
        raise SelectionError("%s: declined must be a list" % source)
    declined = []
    for i, d in enumerate(declined_doc):
