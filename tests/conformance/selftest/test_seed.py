...
class SeedTest(unittest.TestCase):
    ...
    def test_a_partial_group_never_collapses(self):
        cases = [case("s:a/1", Status.FAIL, Verdict.UNEXPECTED_FAILURE),
                 case("s:a/2", Status.PASS, Verdict.AS_EXPECTED)]
        self.assertEqual(seed.propose(cases, "r", False)[0]["matchers"], ["s:a/1"])

    def test_whole_groups_disabled_lists_members(self):
        cases = [case("s:a/1", Status.FAIL, Verdict.UNEXPECTED_FAILURE),
    ...
    def test_append_skips_matchers_already_recorded(self):
        with tempfile.TemporaryDirectory() as tmp:
            leaf = Path(tmp) / "s_b.jsonc"
            action = {"type": "skip", "reason": "r", "matchers": ["s:a/1"]}
            seed.append(leaf, [action])
            before = leaf.read_text()
            seed.append(leaf, [action])
            self.assertEqual(leaf.read_text(), before)
