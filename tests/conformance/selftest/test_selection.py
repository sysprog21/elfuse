...
DATA = {
    ...
    "declined": [{"reason": "unsupported", "groups": ["kernel"]}],
}


class SelectionTest(unittest.TestCase):
    def test_parse(self):
        sel = selection.parse(DATA, "fixture")
        self.assertEqual([e.group for e in sel.groups("pr")], ["basic"])
        self.assertEqual([e.group for e in sel.groups("full")],
                         ["basic", "slow", "narrow"])
        self.assertEqual(sel.entry("slow").timeout_s, 1)
        self.assertEqual(sel.entry("narrow").only, ("keep*",))
        self.assertEqual(sel.declined, [("unsupported", ("kernel",))])
