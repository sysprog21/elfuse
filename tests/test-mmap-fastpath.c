...
static void test_file_mmap_avoids_arena_hole(int flags)
{
    TEST(flags == MAP_SHARED ? "shared mmap avoids live arena hole"
                             : "private mmap avoids live arena hole");
    const size_t chunk = 2ULL << 20;
    char path[] = "/tmp/elfuse-arena-hole-XXXXXX";
    int fd = mkstemp(path);
    if (fd < 0) {
        FAIL("mkstemp");
        return;
    ...
    uint8_t *arena = mmap(NULL, 4 * chunk, PROT_READ | PROT_WRITE,
                          MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (arena == MAP_FAILED) {
        close(fd);
        FAIL("anonymous mmap");
    ...
    arena[0] = 0x31;
    arena[3 * chunk] = 0x42;
    if (munmap(arena + chunk, chunk) != 0) {
        munmap(arena, 4 * chunk);
        close(fd);
        FAIL("munmap arena hole");
        return;
    }

    /* The live neighbors keep the arena from rewinding. A NULL hint leaves its
     * descriptor enabled while the host searches the retired hole.
     */
    uint8_t *file = mmap(NULL, chunk, PROT_READ | PROT_WRITE, flags, fd, 0);
    if (file == MAP_FAILED) {
        munmap(arena, 4 * chunk);
        close(fd);
        FAIL("file mmap");
    ...
    int overlaps = (uintptr_t) file < (uintptr_t) arena + 4 * chunk &&
                   (uintptr_t) file + chunk > (uintptr_t) arena;
    file[0] = 0x53;
    int intact = arena[0] == 0x31 && arena[3 * chunk] == 0x42;
    int rc = munmap(file, chunk);
    close(fd); /* drain any EL1 retirement of the file mapping */
    int cleanup_rc = munmap(arena, 4 * chunk);
    if (overlaps)
        FAIL("file mapping entered the live arena");
    else if (!intact)
        FAIL("live anonymous mappings changed");
    else if (rc != 0 || cleanup_rc != 0)
        FAIL("munmap cleanup");
    else
        PASS();
}
...
static int stats_prefix_hint(void)
{
    ...
    if (r == MAP_FAILED)
        return 1;
    return munmap(r, 4096) != 0 || munmap(q, 4096) != 0 ||
           munmap(p, 65536) != 0;
}

static volatile sig_atomic_t altstack_signal_seen;

static void altstack_signal_handler(int signum)
{
    altstack_signal_seen = signum;
}

static int stats_signal_altstack(bool use_altstack)
{
    ...
    if (p == MAP_FAILED)
        return 1;
    stack_t stack = {.ss_sp = p, .ss_size = size};
    ...
    if (sigaction(SIGUSR1, &action, NULL) != 0 || raise(SIGUSR1) != 0)
        return 1;
    action.sa_handler = altstack_signal_handler;
    action.sa_flags = use_altstack ? SA_ONSTACK : 0;
    if (sigaction(SIGUSR1, &action, NULL) != 0 || raise(SIGUSR1) != 0 ||
        altstack_signal_seen != SIGUSR1)
        return 1;
...
{
    if (strcmp(name, "prefix-hint") == 0)
        return stats_prefix_hint();
    if (strcmp(name, "unused-altstack") == 0)
        return stats_signal_altstack(false);
    if (strcmp(name, "used-altstack") == 0)
        return stats_signal_altstack(true);
...
enum {
    METADATA_WORKERS = 4,
    METADATA_MAPS = 2048,
    METADATA_LENGTH = 2 * 1024 * 1024 + 4096
};
static void *metadata_maps[METADATA_WORKERS][METADATA_MAPS];
static pthread_barrier_t metadata_barrier;
static _Atomic int metadata_full;
...
static void *metadata_worker(void *arg)
{
    ...
    unsigned count = 0;
    int failed = 0;
    pthread_barrier_wait(&metadata_barrier);
    for (; count < METADATA_MAPS; count++) {
        void *p = mmap(NULL, METADATA_LENGTH, PROT_READ | PROT_WRITE,
                       MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
        if (p == MAP_FAILED) {
            if (errno != ENOMEM) {
                fprintf(stderr, "metadata mmap: errno=%d\n", errno);
                failed = 1;
            } else
                atomic_store_explicit(&metadata_full, 1, memory_order_relaxed);
            break;
        }
        metadata_maps[slot][count] = p;
    }

    /* Keep every successful mapping live until all producers exhaust capacity.
     * First touch follows a host exit that drains their publication rings.
     */
    (void) fcntl(-1, F_GETFD);
    pthread_barrier_wait(&metadata_barrier);
    for (unsigned i = 0; i < count; i++) {
        volatile uint8_t *p = metadata_maps[slot][i];
        if (*p != 0)
            failed = 1;
        *p = 0xa5;
        if (*p != 0xa5)
    ...
    pthread_barrier_wait(&metadata_barrier);
    for (unsigned i = 0; i < count; i++)
        if (munmap(metadata_maps[slot][i], METADATA_LENGTH) != 0)
            failed = 1;
    return (void *) (uintptr_t) failed;
}

static int metadata_exhaustion(void)
{
    pthread_t workers[METADATA_WORKERS];
    if (pthread_barrier_init(&metadata_barrier, NULL, METADATA_WORKERS) != 0)
        return 1;
    for (unsigned i = 0; i < METADATA_WORKERS; i++) {
        int rc = pthread_create(&workers[i], NULL, metadata_worker,
                                (void *) (uintptr_t) i);
        if (rc != 0) {
            fprintf(stderr, "metadata pthread_create: %d\n", rc);
            _exit(1);
        }
    }
    int failed = 0;
    for (unsigned i = 0; i < METADATA_WORKERS; i++) {
        void *result;
        if (pthread_join(workers[i], &result) != 0 || result != NULL)
            failed = 1;
    }
    pthread_barrier_destroy(&metadata_barrier);
    if (!atomic_load_explicit(&metadata_full, memory_order_relaxed)) {
        fprintf(stderr, "metadata capacity not exhausted\n");
        failed = 1;
    }
    void *p = mmap(NULL, 4096, PROT_READ | PROT_WRITE,
                   MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    ...
    *(volatile uint8_t *) p = 0x5a;
    return munmap(p, 4096) != 0 || failed;
}

static void test_metadata_exhaustion(void)
{
    TEST("metadata credits under contention");
    pid_t pid = fork();
    if (pid == 0)
        _exit(metadata_exhaustion());
    int status = 0;
    EXPECT_TRUE(pid > 0 && waitpid(pid, &status, 0) == pid &&
                    WIFEXITED(status) && WEXITSTATUS(status) == 0,
                "metadata exhaustion or recovery failed");
}

int main(int argc, char **argv)
{
    if (argc == 2 && strcmp(argv[1], "--metadata-exhaustion") == 0)
        return metadata_exhaustion();
    ...
    test_metadata_exhaustion();
    test_fidelity();
    test_file_mmap_avoids_arena_hole(MAP_PRIVATE);
    test_file_mmap_avoids_arena_hole(MAP_SHARED);
